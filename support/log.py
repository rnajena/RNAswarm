...
def set_log_file(log_file_path, flush_line=False):
    global _log_files
    global _log_file_paths
    _log_file_paths = [log_file_path]
    if flush_line:
        _log_files = [open(log_file_path, 'a', buffering=1)]
    else:
        _log_files = [open(log_file_path, 'a')]
...
def check_level(level):
    return _nameToLevel[level] >= _log_level
